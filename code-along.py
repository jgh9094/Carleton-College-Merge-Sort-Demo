#####################################################################################################
#
# Python script that merge sort and the merger function
# This script was built during and with the class
#
# python3
#####################################################################################################

######################## IMPORTS ########################
import numpy as np

# merge sort
# nums: list of numbers that must be sorted using recursion
def MergeSort(nums):

    # size of list?
    n = len(nums)

    # stopping case
    if n == 1:
        return

    # recursive case
    else:
        first = nums[:n // 2]
        second = nums[n//2 :]

        MergeSort(first)
        MergeSort(second)

        Merge(first, second, nums)

# merger
# l1: 1st half of l3 already sorted
# l2: 2nd half of l3 already sorted
# l3: merged list in sorted order
def Merge(l1,l2,l3):
    # print('Merging l1 & l2 into l3!')
    count1,count2,count3 = 0,0,0

    while count1 < len(l1) and count2 < len(l2):
        if l1[count1] < l2[count2]:
            l3[count3] = l1[count1]
            count1 += 1
        else:
            l3[count3] = l2[count2]
            count2 += 1
        count3 += 1

    # empty out l1
    while count1 < len(l1):
        l3[count3] = l1[count1]
        count1 += 1
        count3 += 1

    # empty out l2
    while count2 < len(l2):
        l3[count3] = l2[count2]
        count2 += 1
        count3 += 1

def main():
    # size of list we are sorting
    size = 8
    # lower & upper limit on random numbers possible
    lower = 0
    upper = 100
    # get random list of numbers
    nums = np.random.randint(lower, upper, size)
    left = np.sort(np.random.randint(lower, upper, size // 2))
    right = np.sort(np.random.randint(lower, upper, size // 2))
    # combine both lists
    together = [*left, *right]
    # prints lists
    print('left: ', left)
    print('right: ', right)
    print('together:', together)
    print()

    # merge sort both
    Merge(left, right, together)
    print('merge: ', together)
    print()

    # sort the list of nums!
    print('nums before: ', nums)
    MergeSort(nums)
    print('nums after: ', nums)
    print()

    # make sure the list is sorted
    print('sorted:', all(nums[i] <= nums[i+1] for i in range(len(nums) - 1)))
    print(nums)

if __name__ == "__main__":
    main()